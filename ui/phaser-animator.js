...
export class PhaserAnimator {
     ...
     */
    async animateSwap(sprite1, sprite2) {
     ...
     */
    async animateInvalidSwap(sprite1, sprite2) {
        return new Promise((resolve) => {
            const pos1 = { x: sprite1.x, y: sprite1.y };
            const pos2 = { x: sprite2.x, y: sprite2.y };
     ...
     */
    async animateTileClearing(positions, tileSprites) {
        return new Promise((resolve) => {
     ...
     */
    async animateFallingTiles(movements, tileSprites) {
        if (movements.size === 0)
        ...
        return new Promise((resolve) => {
            ...
            movements.forEach((newPos, oldPosKey) => {
                const [oldRow, oldCol] = oldPosKey.split(',').map(Number);
     ...
     */
    async animateSpawningTiles(tileSprites, boardState) {
        return new Promise((resolve) => {
            const spawnPromises = [];
     ...
     */
    createNewTileSprite(row, col, tileType) {
     ...
     */
    async animateTileSpawn(sprite, targetRow) {
        return new Promise((resolve) => {
...
//# sourceMappingURL=phaser-animator.js.map
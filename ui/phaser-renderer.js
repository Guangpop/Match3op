...
export class Match3Scene extends Phaser.Scene {
    ...
    createBackground() {
        // Game background
        this.add.rectangle(this.cameras.main.centerX, this.cameras.main.centerY, this.cameras.main.width, this.cameras.main.height, 0x2C3E50);
        // Board background
        this.backgroundGrid = this.add.graphics();
        this.backgroundGrid.fillStyle(0x34495E);
        this.backgroundGrid.fillRoundedRect(this.BOARD_OFFSET_X - 10, this.BOARD_OFFSET_Y - 10, this.BOARD_SIZE * this.TILE_SIZE + 20, this.BOARD_SIZE * this.TILE_SIZE + 20, 10);
        // Grid lines
        this.backgroundGrid.lineStyle(1, 0x5D6D7E, 0.5);
        for (let i = 0; i <= this.BOARD_SIZE; i++) {
            // Vertical lines
            this.backgroundGrid.lineBetween(this.BOARD_OFFSET_X + i * this.TILE_SIZE, this.BOARD_OFFSET_Y, this.BOARD_OFFSET_X + i * this.TILE_SIZE, this.BOARD_OFFSET_Y + this.BOARD_SIZE * this.TILE_SIZE);
            // Horizontal lines
            this.backgroundGrid.lineBetween(this.BOARD_OFFSET_X, this.BOARD_OFFSET_Y + i * this.TILE_SIZE, this.BOARD_OFFSET_X + this.BOARD_SIZE * this.TILE_SIZE, this.BOARD_OFFSET_Y + i * this.TILE_SIZE);
        }
    }
    createUI() {
        // Title
    ...
    handleTileClick(_pointer, gameObject) {
        if (this.isAnimating)
            return;
    ...
    async attemptSwap(pos1, pos2) {
        ...
        if (!matches) {
            ...
            await this.animator.animateInvalidSwap(this.tileSprites[pos1.row][pos1.col], this.tileSprites[pos2.row][pos2.col]);
            return;
        }
        ...
        // Animate the swap
        await this.animator.animateSwap(this.tileSprites[pos1.row][pos1.col], this.tileSprites[pos2.row][pos2.col]);
        ...
        logManager.logPlayerMove(logManager.getMoveLogs().length + 1, { pos1, pos2, tile1Type, tile2Type }, boardStateBefore, matches.matches.map(m => m.positions), cascadeEvents, boardStateAfter, scoreGained, this.score);
    ...
    */
    async processCascadingMatchesWithLogging() {
        const cascadeEvents = [];
        let cascadeLevel = 1;
        while (true) {
            const boardState = this.boardManager.getBoard();
            ...
            // Calculate score with cascade multiplier
            const cascadeScore = this.matchEngine.calculateScoreWithMultiplier(matchResult.clearedPositions.length, cascadeLevel);
            this.score += cascadeScore;
            this.updateStatus(`Cascade level ${cascadeLevel}! +${cascadeScore} points`);
            ...
            const cascadeEvent = logManager.createCascadeEvent(cascadeLevel, matchResult.clearedPositions, cascadeScore, fallingTiles, newTilesSpawned, boardAfterRefill);
    ...
    showHint() {
        ...
        // Show best move
        const bestMove = validMoves.reduce((best, current) => current.matches.clearedPositions.length > best.matches.clearedPositions.length
            ? current
            : best);
...
//# sourceMappingURL=phaser-renderer.js.map
...
export class LogViewer {
  ...
  private updateStats(): void {
    ...
    if (sessionIdEl) sessionIdEl.textContent = logManager.getSessionId();
    if (totalMovesEl) totalMovesEl.textContent = stats.totalMoves.toString();
    if (totalScoreEl) totalScoreEl.textContent = stats.totalScore.toString();
    if (loggingStatusEl) loggingStatusEl.textContent = logManager.isLoggingEnabled() ? 'Enabled' : 'Disabled';
   ...
   */
  private updateCurrentLogs(): void {
    if (!this.container) return;
  ...
  showLogViewerModal(): void {
    ...
    const container = modal.querySelector('#modal-log-container') as HTMLElement;
    if (container) {
